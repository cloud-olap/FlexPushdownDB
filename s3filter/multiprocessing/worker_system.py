...
class WorkerSystem(object):
    ...
    def __init__(self):
        self.workers = {}
        self.channel = Channel(200000, 200)

    def create_worker(self, name, shared_array_size, buffer_item_size, handler):
        worker = Worker(name, shared_array_size, buffer_item_size, self, handler)
    ...
    def create_message(self, messsage_type, data):
        ...
        if multiprocessing.current_process().name == 'MainProcess':
            msg = MessageBase(messsage_type, 'system', data)
            return msg
        else:
