...
def test_parallel_unbuffered_sharded():
    run(parallel=True, use_pandas=True, buffer_size=0, lineitem_parts=4, part_parts=2, sharded=False)
...
def run(parallel, use_pandas, buffer_size, lineitem_parts, part_parts, sharded):
    ...
    part_scan = map(lambda p:
                    query_plan.add_operator(
                        tpch_q19.sql_scan_part_partkey_brand_size_container_where_extra_filtered_op(
                            sharded,
                            p,
                            part_parts,
                            use_pandas,
