...
def run(group_fields, agg_fields, parallel, use_pandas, buffer_size, table_parts):
    ...
    scan_phase1 = map(lambda p: 
               query_plan.add_operator(
                    SQLTableScan('groupby_benchmark/shards-10GB/groupby_data_{}.csv'.format(p),
                        "select {} from S3Object;".format(','.join(group_fields)), use_pandas,
    ...
    groupby_filter_build = query_plan.add_operator(
                     GroupbyFilterBuild( agg_exprs, 'groupby_filter_build', query_plan, False)) 
    ...
    map(lambda (p, o): groupby_filter_build.connect(o), enumerate(scan_phase2))
