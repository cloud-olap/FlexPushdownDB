...
def main():
    ...
    run(True, True, 0, table_first_part=1, table_parts=2, queried_columns=queried_columns, 
        select_columns=select_columns, lower=0, upper=910, path=path, format_=Format.CSV)


def run(parallel, use_pandas, buffer_size, table_first_part, table_parts, queried_columns, queried_aliases, castable_aliases, select_str, aggregate_column, filter_str, path, format_=Format.CSV):
    secure = False
    use_native = False
    print('')
    print("CSV Aggregate Benchmark")
    print("------------------")
    ...
    scan = map(lambda p:
               query_plan.add_operator(
                   SQLTableScan("{}/lineitem.tbl.{}".format(path, p),
                                ...
                                "{}".format(select_str, filter_str), format_,
