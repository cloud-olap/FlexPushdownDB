...
bool ProjectPrePOp::equalTo(const std::shared_ptr<PrePhysicalOp> &other) const {
  // self
  if (type_ != other->getType()) {
    return false;
  }
  auto typedOther = std::static_pointer_cast<ProjectPrePOp>(other);
  if (!(expression::gandiva::Expression::equals(exprs_, typedOther->exprs_) &&
       exprNames_ == typedOther->exprNames_ &&
       projectColumnNamePairs_ == typedOther->projectColumnNamePairs_)) {
    return false;
  }
  // producers
