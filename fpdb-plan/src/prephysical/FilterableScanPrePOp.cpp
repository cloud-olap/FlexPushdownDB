...
bool FilterableScanPrePOp::equalTo(const std::shared_ptr<PrePhysicalOp> &other) const {
  // self
  if (type_ != other->getType()) {
    return false;
  }
  auto typedOther = std::static_pointer_cast<FilterableScanPrePOp>(other);
  if (!(table_->getName() == typedOther->table_->getName() &&
