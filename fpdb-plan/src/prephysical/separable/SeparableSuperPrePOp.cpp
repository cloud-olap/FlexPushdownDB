...
bool SeparableSuperPrePOp::equalTo(const std::shared_ptr<PrePhysicalOp> &other) const {
  // self
  if (type_ != other->getType()) {
    return false;
  }
  auto typedOther = std::static_pointer_cast<SeparableSuperPrePOp>(other);
  if (!equals(rootOp_, typedOther->rootOp_)) {
    return false;
  }
  // producers
