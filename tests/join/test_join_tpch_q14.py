...
def test_join_baseline():
    """TPC-H Q14
    This uses nested loop joins with no projection and no filtering.

    Note that it uses a limit clause to make testing speeds sane.
    ...
    ts2 = TableScan('part.csv',
                    ...
                    ";")
    f1 = Filter(PredicateExpression(lambda t: cast(t['_10'], timestamp) >= cast(min_shipped_date, timestamp)))
    ...
    assert c.tuples()[1] == ['1', '155190', '7706', '1', '17', '21168.23', '0.04', '0.02', 'N', 'O', '1996-03-13', '1996-02-12', '1996-03-22', 'DELIVER IN PERSON', 'TRUCK', 'egular courts above the', '155190', 'chocolate lace cornflower rosy light', 'Manufacturer#4', 'Brand#44', 'PROMO BRUSHED NICKEL', '9', 'JUMBO JAR', '1245.19', 'sly unusual orbits use']
    assert c.tuples()[2] == ['1', '67310', '7311', '2', '36', '45983.16', '0.09', '0.06', 'N', 'O', '1996-04-12', '1996-02-28', '1996-04-20', 'TAKE BACK RETURN', 'MAIL', 'ly final dependencies: slyly bold ', '67310', 'drab sienna black thistle smoke', 'Manufacturer#3', 'Brand#34', 'STANDARD BURNISHED STEEL', '47', 'MED BAG', '1277.31', 'dependencies. furiou']
    assert c.tuples()[3] == ['1', '63700', '3701', '3', '8', '13309.60', '0.10', '0.02', 'N', 'O', '1996-01-29', '1996-03-05', '1996-01-31', 'TAKE BACK RETURN', 'REG AIR', 'riously. regular, express dep', '63700', 'saddle green lace coral papaya', 'Manufacturer#5', 'Brand#53', 'MEDIUM BRUSHED COPPER', '16', 'JUMBO CAN', '1663.70', 'p fluffily blithe']
