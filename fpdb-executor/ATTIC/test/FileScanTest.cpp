...
#include <fpdb/pushdown/project/Project.h>
...
TEST_CASE ("filescan-sum-collate" * doctest::skip(true || SKIP_SUITE)) {
  ...
  auto fileScan = fpdb::pushdown::file::FileScan::make("fileScan", "data/filescan/single-partition/test.csv", std::vector<std::string>{"A"}, 0, numBytesTestFile, g->getId());
  ...
  aggregateFunctions->emplace_back(std::make_shared<Sum>("Sum",
														 cast(fpdb::expression::gandiva::col("A"), float64Type())));

  auto aggregate = std::make_shared<fpdb::pushdown::aggregate::Aggregate>("aggregate", aggregateFunctions);
  auto collate = std::make_shared<fpdb::pushdown::collate::Collate>("collate", g->getId());
...
TEST_CASE ("filescan-project-collate" * doctest::skip(false || SKIP_SUITE)) {
  ...
  auto fileScan = fpdb::pushdown::file::FileScan::make("fileScan", "data/filescan/single-partition/test.csv", columnNames , 0, 18, g->getId());
  auto expressions = {
	  cast(fpdb::expression::gandiva::col("A"), float64Type()),
	  fpdb::expression::gandiva::col("B")
  };
  auto project = std::make_shared<fpdb::pushdown::project::Project>("project", expressions);
  auto collate = std::make_shared<fpdb::pushdown::collate::Collate>("collate", g->getId());
...
TEST_CASE ("filescan-sum-collate-parallel" * doctest::skip(true || SKIP_SUITE)) {
  ...
  auto fileScan01 = fpdb::pushdown::file::FileScan::make("fileScan01", "data/filescan/multi-partition/test01.csv", std::vector<std::string>{"A"}, 0, numBytesTest01File, g->getId());
  auto fileScan02 = fpdb::pushdown::file::FileScan::make("fileScan02", "data/filescan/multi-partition/test02.csv", std::vector<std::string>{"A"}, 0, numBytesTest02File, g->getId());
  auto fileScan03 = fpdb::pushdown::file::FileScan::make("fileScan03", "data/filescan/multi-partition/test03.csv", std::vector<std::string>{"A"}, 0, numBytesTest03File, g->getId());
  ...
  auto aggregate01 = std::make_shared<fpdb::pushdown::aggregate::Aggregate>("aggregate01", expressions01);
  ...
  expressions02->emplace_back(std::make_shared<Sum>("sum(A)",
													cast(col("A"), float64Type())));
  auto aggregate02 = std::make_shared<fpdb::pushdown::aggregate::Aggregate>("aggregate02", expressions02);
  ...
  expressions03->emplace_back(std::make_shared<Sum>("sum(A)",
													cast(col("A"), float64Type())));
  auto aggregate03 = std::make_shared<fpdb::pushdown::aggregate::Aggregate>("aggregate03", expressions03);
  ...
  auto reduceAggregate = std::make_shared<fpdb::pushdown::aggregate::Aggregate>("reduceAggregate", reduceAggregateExpressions);
