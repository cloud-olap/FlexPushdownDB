...
#include <fpdb/pushdown/group/Group.h>
...
TEST_CASE ("group-filescan-group-collate" * doctest::skip(false || SKIP_SUITE)) {
  {
	...
	auto scan = file::FileScan::make("fileScan",
							   ...
							   true);
	g->put(scan);
	auto sumExpr = std::make_shared<fpdb::pushdown::aggregate::Sum>("sum",
																	  cast(col("AB"), float64Type()));
	auto
		expressions2 =
		std::make_shared<std::vector<std::shared_ptr<fpdb::pushdown::aggregate::AggregationFunction>>>();
