...
TEST_CASE ("ssb-query2.1-sf0.01-file-par1-partfilter" * doctest::skip(false || SKIP_SUITE)) {
  auto n = Normal::start();
  LocalFileSystemTests::partFilter("MFGR#12", "data/ssb-sf0.01", 1, true, n);
  n->stop();
}

TEST_CASE ("ssb-query2.1-sf0.01-file-par1-join2x" * doctest::skip(false || SKIP_SUITE)) {
  auto n = Normal::start();
  LocalFileSystemTests::join2x("AMERICA", "data/ssb-sf0.01", 1, true, n);
  n->stop();
}

TEST_CASE ("ssb-query2.1-sf0.01-file-par1-join3x" * doctest::skip(false || SKIP_SUITE)) {
  auto n = Normal::start();
  LocalFileSystemTests::join3x("AMERICA", "data/ssb-sf0.01", 1, true, n);
  n->stop();
}

TEST_CASE ("ssb-query2.1-sf0.01-file-par1-join" * doctest::skip(false || SKIP_SUITE)) {
  auto n = Normal::start();
  LocalFileSystemTests::join("MFGR#12", "AMERICA", "data/ssb-sf0.01", 1, true, n);
  n->stop();
}
