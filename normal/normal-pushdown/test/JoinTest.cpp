...
TEST_CASE ("join-test-filescan-join-collate" * doctest::skip(false)) {
  ...
  auto aScan = FileScan::make("fileScanA", "data/join/a.csv", std::vector<std::string>{"AA", "AB", "AC"}, 0, numBytesAFile, g->getId(), true);
  auto bScan = FileScan::make("fileScanB", "data/join/b.csv", std::vector<std::string>{"BA", "BB", "BC"}, 0, numBytesBFile, g->getId(), true);
